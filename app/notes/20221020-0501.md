...
4.  Run `sudo parted -l` and parted should ask to automatically fix the size mismatch.  Fix the size mismatch.  Note that this only corrects the GPT PMBR size mismatch; it does not expand the partition.
5.  Run `sudo parted /dev/vda resizepart 3 100%` to expand the partition into the new free space.
